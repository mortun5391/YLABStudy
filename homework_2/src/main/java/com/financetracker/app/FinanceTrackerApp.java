...
import com.financetracker.config.DataSourceConfig;
import com.financetracker.config.LiquibaseConfig;
...
public class FinanceTrackerApp {
    ...
    public static void main(String[] args) {
        ...
        LiquibaseConfig.runMigrations(dataSource);

        // Создание репозиториев с передачей DataSource
        UserRepository userRepository = new UserRepository(dataSource);
        TransactionRepository transactionRepository = new TransactionRepository(dataSource);
        BudgetRepository budgetRepository = new BudgetRepository(dataSource);
    ...
    private static void removeTransaction() {
        long id = inputValidator.getIntInput("Введите ID транзакции для удаления: ");
    ...
    private static void editTransaction() {
        long id = inputValidator.getIntInput("Введите ID транзакции для редактирования: ");
     ...
     */
    private static void viewTransactions(long id) {
        while (true) {
...
// ADMIN FEATURES
    ...
    private static void viewUsersList() {
        ...
        while (true) {
            ...
            switch (choice) {
                case 1:
                    long id = inputValidator.getIntInput("Введите id пользователя: ");
                    if (financeTracker.isUserExist(id)) viewTransactions(id);
                    else System.out.println("Пользователь с таким id не найден!");

                    break;
                case 2:
                    id = inputValidator.getIntInput("Введите id пользователя: ");
